...
function findLastIndex(array, predicate, fromIndex) {
  ...
  if (fromIndex !== undefined) {
    ...
    index = fromIndex < 0 ? nativeMax$3(length + index, 0) : nativeMin(index, length - 1);
  }
...
function lastIndexOf(array, value, fromIndex) {
  ...
  if (fromIndex !== undefined) {
    index = toInteger(fromIndex);
    index = index < 0 ? nativeMax$5(length + index, 0) : nativeMin$2(index, length - 1);
  }
...
function baseSortedIndexBy(array, value, iteratee, retHighest) {
  ...
  return nativeMin$3(high, MAX_ARRAY_INDEX);
}
 ...
 */
var createSet = !(Set && 1 / setToArray(new Set([, -0]))[1] == INFINITY$4) ? noop : function (values) {
...
function composeArgsRight(args, partials, holders, isCurried) {
  var argsIndex = -1,
    ...
    rangeLength = nativeMax$8(argsLength - holdersLength, 0),
...
function getFuncName(func) {
  var result = func.name + '',
    ...
    length = hasOwnProperty$10.call(realNames, result) ? array.length : 0;
...
function reorder(array, indexes) {
  var arrLength = array.length,
    length = nativeMin$4(indexes.length, arrLength),
...
function createWrap(func, bitmask, thisArg, partials, holders, argPos, ary, arity) {
  ...
  ary = ary === undefined ? ary : nativeMax$9(toInteger(ary), 0);
  ...
  partials = newData[3];
  holders = newData[4];
  arity = newData[9] = newData[9] === undefined ? isBindKey ? 0 : func.length : nativeMax$9(newData[9] - length, 0);
...
function createRound(methodName) {
  ...
  return function (number, precision) {
    ...
    precision = precision == null ? 0 : nativeMin$6(toInteger(precision), 292);
    if (precision) {
...
function includes(collection, value, fromIndex, guard) {
  ...
  var length = collection.length;
  if (fromIndex < 0) {
    fromIndex = nativeMax$10(length + fromIndex, 0);
  }
...
function debounce(func, wait, options) {
  ...
  function remainingWait(time) {
    ...
    return maxing ? nativeMin$7(timeWaiting, maxWait - timeSinceLastInvoke) : timeWaiting;
  }
...
var defaults$1 = baseRest(function (object, sources) {
  ...
  while (++index < length) {
    ...
    while (++propsIndex < propsLength) {
      ...
      if (value === undefined || eq(value, objectProto$21[key]) && !hasOwnProperty$18.call(object, key)) {
...
function baseInRange(number, start, end) {
  return number >= nativeMin$8(start, end) && number < nativeMax$12(start, end);
}
...
function toSafeInteger(value) {
  return value ? baseClamp(toInteger(value), -MAX_SAFE_INTEGER$3, MAX_SAFE_INTEGER$3) : value === 0 ? value : 0;
}
...
var overArgs = castRest(function (func, transforms) {
  ...
  return baseRest(function (args) {
    var index = -1,
      length = nativeMin$9(args.length, funcsLength);
...
function pad(string, length, chars) {
  ...
  return createPadding(nativeFloor$3(mid), chars) + string + createPadding(nativeCeil$2(mid), chars);
}
...
function random(lower, upper, floating) {
  ...
  if (floating || lower % 1 || upper % 1) {
    ...
    return nativeMin$10(lower + rand * (upper - lower + freeParseFloat('1e-' + ((rand + '').length - 1))), upper);
  }
...
function baseRange(start, end, step, fromRight) {
  var index = -1,
    length = nativeMax$13(nativeCeil$3((end - start) / (step || 1)), 0),
    result = Array(length);
...
function spread(func, start) {
  ...
  start = start == null ? 0 : nativeMax$14(toInteger(start), 0);
  return baseRest(function (args) {
